...
class XoopsFormAlphaBarreTbl extends XoopsFormAlphaBarre
{
  var $path = '';
  var $url = '';
  function __construct($idAlphabarre, $name,  
                       $link,  
                       $letter = "",
                       $page = 1,
                       $limit = 0,
    ...
    global $xoopsDB;
      ...
      $this->url = str_replace(JANUS_PATH, JANUS_URL, $this->path);
    
    
    $sql = "SELECT * FROM ".$xoopsDB->prefix("jjd_alphabarre")
         . " WHERE idAlphabarre=" . $idAlphabarre;
    $rst = $xoopsDB->query($sql);
    $t = $xoopsDB->fetchArray($rst);
echoArray($t, 'XoopsFormAlphaBarreTbl');    
    XoopsFormAlphaBarre::__construct($name,$link,$letter,$page,$limit,$signet);
    
//     if ($t['alphabet'] != ''){
//         $this->setVar('alphabet',$t['alphabet']);
//     }else{
//       $this->setAlphabet(0);
//     }
    
    $this->setVar('other',$t['other']);
    $this->setVar('showLettersUnused',$t['showAllLetters']);
    //$this->setVar('frameDelimitor',$t['frameDelimitor']);
    //$this->setVar('frameDelimitor',$t['frameDelimitor']);
//     $this->setVar('letterSeparator',$t['letterSeparator']);
//     $this->setVar('rows',$t['rows']);
//     $this->setVar('imgFolder',$t['imgFolder']);
//     $this->setVar('imgScale',$t['imgScale']);
//     $this->setVar('fontName',$t['font']);
//     $this->setVar('fontSize',$t['size']);
//     $this->setVar('fontColor',$t['color']);
//     $this->setVar('fontColorOver',$t['colorSelected']);
    //$this->setVar('template',$t['template']);
//     $this->setVar('',$t['']);
//     $this->setVar('',$t['']);
//     $this->setVar('',$t['']);
//     $this->setVar('',$t['']);
...
//////////////////////////////////////////////////////////////////
class XoopsFormAlphaBarreLight extends XoopsFormAlphaBarre
{
  ...
  function __construct($link,  
                       ...
                       $page = 1,
                       $limit = 0){
    XoopsFormAlphaBarre::__construct('',$link,$letter,$page,$limit,'');
  } 
}
 ...
 *****************************************/
  function init (){
   ...
   $this->_vars = array('name'               => 'alphaBarre',
                     ...
                     'interLigne'         => 2, //en px <br />
                     'extrabarre_opt'     => array(),
                     'extrabarre'         => ''
                     );
 ...
 *****************************************/
  ...
  $t = array();
    ...
    $value = implode('', $t)
           ...
           . $this->_getLetterIfExist($this->_vars['allById']);
 ...
 *
 *****************************************/
  function setInfoTable ($table, $fieldName, $fieldId = '', $filtre='', $orderby=''){ 
    $this->setVar('table', $table);
    $this->setVar('fieldName', $fieldName);
    $this->setVar('fieldId', $fieldId);
...
//echoArray($this->_vars, 'setInfoTable');  
...
//echoArray($this->_vars,'getCriteria');  
    $fieldName = $this->_vars['fieldName'];
    $fieldId = $this->_vars['fieldId'];
    $letter = $this->_vars['letter'];
    $notin = $this->_vars['notin'];
    $all = $this->_vars['all'];
    $allById = $this->_vars['allById'];
    $filter = $this->_vars['filtre'];  
    $orderby = $this->_vars['orderby'];  
    //----------------------------------------
    $t = array();
    $t['orderby'] = $orderby;
    ...
    if($letter=='' || $letter == $notin){
      ...
      $t['criteria'] = "";
    }elseif($letter == $allById){
      $t['criteria'] = "";
      $t['orderby'] = $fieldId;
    }else{
      $t['criteria'] = "{$fieldName} LIKE '{$letter}%'";
      if ($filter != '') $t['criteria'] .= ' AND ' . $filter;
    }
  
    //-------------------------------------------------------------------
    if($t['criteria'] == ''){
      if ($filter != '') $t['criteria'] = $filter;
    }else{
      if ($filter != '') $t['criteria'] .= ' AND ' . $filter;
    }
    //-------------------------------------------------------------------
    ...
    if (is_numeric ($alphabet)){
      switch ($alphabet){
        ...
        case 3: 
          $this->_vars['notin'] = "#";                  
          //$this->_vars['allById'] = "$";                  
          $alphabet = "{$this->_vars['notin']}ABCDEFGHIJKLMNOPQRSTUVWXYZ{$this->_vars['all']}{$this->_vars['allById']}";                  
          break;
        default:
 ...
 *****************************************/
  function toString($sep = ',', $car2exclude = null){ 
    if (!$car2exclude) 
    $car2exclude = $this->_vars['notin'].$this->_vars['all'].$this->_vars['allById'];
    //$car2exclude = $this->_vars['notin'].$car2exclude = $this->_vars['all'];
 ...
 *
 *****************************************/
  function add_extrabarre ($key, $caption, $title,  $prompt_lib=''){
    $this->_vars['extrabarre_opt'][$key] = array('caption'=>$caption, 'title'=>$title, 'prompt_lib'=>$prompt_lib);
  }
...
private function _getLink(){
  ...
  //-------------------------------------------------------
  //si le nom a ete defini dans le constructeur
  //l'url renverra un tableau dans le get sinon une suite classique de paramettre
  // - ?letter=A&page=5&signet=
  // ou
  // - ?name[letter]=A&name[page]=5&name[signet]=
  //-------------------------------------------------------
  $idForSubmit = $this->getVar('name');
  if ($idForSubmit == '') $idForSubmit = 'alphaBarre';
  
  //si il n'y a pas d lien dfini c'est qu'il y a une balise forme dans ce cas ce sera un submit
  if ($link0 == ''){
    $link1 = "<a class='%class%' href='javascript: alphabarre_submit2(\"{$idForSubmit}\", \"%letter%\",\"%page%\",\"%extrabarre%\");' >%visu%</a>";
    //$link1 = "<a class='%class%' href='' onclick='alphabarre_submit(event,this);' letter='%letter%' page='%page%'>%visu%</a>";
  }else{
    if($this->_vars['arrayParamName'] == ''){
    $link0 .= ((stripos($link0, '?') === false ) ? '?' : '&');
      $link1 = "<a class='%class%' href='" . $link0 
             . "{$this->_vars['letterParamName']}=%letter%" 
             . '&' . "{$this->_vars['pageParamName']}=%page%"
             . '&' . "{$this->_vars['signetParamName']}=%signet%"
             . "' >%visu%</a>";
    }else{
      $link1 = "<a class='%class%' href='" . $link0 
...
private function _render_barre(){ 
  ...
  for ($h=0; $h < count($ta); $h++){
    ...
    //return implode('', $ta);
    return "<div style='margin-top:{$this->_vars['interLigne']}px;'>" . implode('', $ta) . "</div>";
...
//jecho($this->_vars,'zzzzzzzzzzzzzzzzzzzzzzzz') ;
//jexit;        
...
//         $visu =  "<font color='{$colorSelection}'>" .$visu. "</font>";
      ...
      $link3 = str_replace('%extrabarre%', $this->_vars['extrabarre'], $link3);
      $tl[] = $link3;  
   ...
   //return $first . $previous . $blank . $htmlPages . $blank . $next . $last;
   return "<div style='margin-top:{$interLigne}px;'>" . $first . $previous . $blank . $htmlPages . $blank . $next . $last."</div>";
 ...
 *
 ****************************************************************************/
private function _render_extrabarre($idForSubmit){ 
  $t = explode('|', $this->_vars['extrabarre']);
  $current_code = $t[0];
  //jecho( $this->_vars['extrabarre_opt']);  
  //jecho( $this->_vars);
  
  //$link1 = $this->_vars['link1'];
//   $link1 = "<a class='%class%' href='javascript: alphabarre_submit_extrabarre(\"{$idForSubmit}\", \"%letter%\",\"%page%\", \"%filter%\", \"%caption%\");' >%visu%</a>";
//   $link1 = "|a class='%class%' href='javascript: alphabarre_submit_extrabarre(\"{$idForSubmit}\", \"%letter%\",\"%page%\", \"%filter%\", \"%caption%\");' |%visu%|/a|";
  
  $link1 = "<a class='%class%' href='javascript: alphabarre_submit_extrabarre(\"{$idForSubmit}\", \"%letter%\",\"%page%\", \"%extrabarre%\", \"%prompt_lib%\");' title='%title%'>%visu%</a>";
  //$link1 = "|a class='%class%' href='javascript: alphabarre_submit_extrabarre(\"{$idForSubmit}\", \"%letter%\",\"%page%\", \"%extrabarre%\", \"%caption%\");' |%visu%|/a|";
  
  $link1 = str_replace ('%page%',$this->_vars['page'], $link1); 
  $link1 = str_replace ('%letter%',urlencode($this->_vars['letter']), $link1);

  
  $tHtml = array();
  foreach ($this->_vars['extrabarre_opt'] as $key => $t){
    //$tHtml[] = "<a href='{}' onclick='alphabarre_onextrabarre();'>{}</a>";
    //$link1 = "<a class='%class%' href='javascript: alphabarre_submit2(\"{$idForSubmit}\", \"%letter%\",\"%page%\");' >%visu%</a>";
    //$link1 = "<a href='javascript: alphabarre_submit2(\"{$idForSubmit}\", \"%letter%\",\"%page%\", \"%filter%\", \"%caption%\");' >%visu%</a>";
    $link2 = str_replace ('%extrabarre%',urlencode($key), $link1);
    $link2 = str_replace ('%visu%',urlencode($t['caption']), $link2);
    $link2 = str_replace ('%title%',$t['title'], $link2);
    $link2 = str_replace ('%prompt_lib%',$t['prompt_lib'], $link2);
  ...
  //$html = implode('<br />', $tHtml);
  $html = implode($this->_vars['separator'], $tHtml);
  if ($this->_vars['delimitorAlphabet'] != ''){
     $html  = str_replace("@", $html, $this->_vars['delimitorAlphabet']);
  }
...
//-----------------------------------
     ...
     $xoTheme->addScript($this->url . '/alphabarre.js');
//-------------------------------------  
 //jecho($this->_vars); 
  ...
  $idForSubmit = (($name == '') ? 'alphaBarre' : $name);
  //$extrabarre='';
  if($name==''){
    $tHtml[] = "<input type='hidden' name='letter' id='{$idForSubmit}_letter' value='{$letter}' />";
    $tHtml[] = "<input type='hidden' name='page'   id='{$idForSubmit}_page' value='{$page}' />";
    $tHtml[] = "<input type='hidden' name='maxPage'   id='{$idForSubmit}_maxPage' value='{$maxPage}' />";
    $tHtml[] = "<input type='hidden' name='nbPages'   id='{$idForSubmit}_nbPages' value='{$pages}' />";
    $tHtml[] = "<input type='hidden' name='extrabarre' id='{$idForSubmit}_extrabarre' value='{$extrabarre}' />";
  }else{
    $tHtml[] = "<input type='hidden' name='{$name}[letter]' id='{$idForSubmit}_letter' value='{$letter}' />";
    $tHtml[] = "<input type='hidden' name='{$name}[page]'   id='{$idForSubmit}_page' value='{$page}' />";
    $tHtml[] = "<input type='hidden' name='{$name}[extrabarre]'   id='{$idForSubmit}_extrabarre' value='{$extrabarre}' />";
  }
  $tHtml[] = "";
  
  $tHtml2[] = "<div align='center' width='100%'>";
  if ($this->_vars['caption'] != ''){
    $tHtml2[] = "<center><span>".$this->_vars['caption']."</span></center>";
  }
  
  $this->_getLink();
  $tHtml2[] = "<font face='{$fontName}' size='{$fontSize}' color='{$fontColor}'>";
  
  //=================================================================
  // generation de la barre de lettres
  //=================================================================
  if ($this->_vars['showAlphaBarre']){
      $tHtml2[]  = $this->_render_barre();
  } 
  ...
  //=================================================================
  if ($pages > 0) {
     //if ($this->_vars['showAlphaBarre']) $tHtml2[]  = $interLigne ;
     //$tHtml2[]  = $interLigne . $first . $previous . $blank . $htmlPages . $blank . $next . $last;
     $tHtml2[]  = $this->_render_pages();
  }  
  
  //=================================================================
  //traitement des extrabarres sil il y en a
  //=================================================================
  if ( count($this->_vars['extrabarre_opt']) > 0) {
     //if ($this->_vars['showAlphaBarre']) $tHtml2[]  = $interLigne ;
     //$tHtml2[]  = $interLigne . $first . $previous . $blank . $htmlPages . $blank . $next . $last;
     $tHtml2[]  = $this->_render_extrabarre($idForSubmit);
  }  
  
  $tHtml2[] = '</font></div>';  
  
  $barreBis = implode('', $tHtml2);
  $tHtml[] = $barreBis;  
...
//-------------------------------------  
  ...
  if ($this->_vars['caption'] != ''){
    $tHtml[] = "<center><span>".$this->_vars['caption']."</span></center>";
  }

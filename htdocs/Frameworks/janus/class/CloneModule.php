...
function clone($clone){
    $dirName   = $this->module->getInfo('dirname');
    $imagePath = $this->module->getInfo('image');
...
//   $xoopsTpl->display($this->path . "/cloneModule.tpl");  
    ...
    $msg = $this->result($clone);
  
  
    $form  = new \XoopsThemeForm(\sprintf("Module %s", $clone), 'clone', $url, 'post', true);
    $inpMsg = new \XoopsFormLabel("", $msg);
    //$inpMsg->setDescription("ddddddddddddddddddd");
    $form->addElement($inpMsg);

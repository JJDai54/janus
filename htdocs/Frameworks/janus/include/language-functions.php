...
function language2Array($file){
  $text = \JANUS\FSO\loadTextFile($file);
  $tDef = \JANUS\findDefinition($file);
  ...
  foreach($tDef as $key=>$def){
    //echo "--->{$def}<br>"; 
    $newDef = \JANUS\parseDefinition(($def));
  //echoArray($newDef);
...
//echo "<hr>lg = {$fileFulName}<hr>";
  ...
  return \JANUS\language2Array($fileFulName);
}
/**************************************************************
 *
 **************************************************************/ 
...
function findDefinition($fileFulName){
  $text = \JANUS\FSO\loadTextFile($fileFulName);
  //echo "<pre>{$text}</pre>";
